...
You can run multiple equations in a code block too
Example: 
64**(1/3)
17*43
[1] 4
...
* When constructing a vector, R will coerce values to the most general type if 
it encounters values of different types:
...
```{r run_vector_addition}
c(1,2) + c(3,4)
```
```{r run_vector_multiplication}
c(1,2) * c(3,4)
```
...
Run:
```{r run_matrix_setup}
# creates a matrix with two rows and three columns containing integers
...
m
```
```{r run_matrixNamedRow}
m["r1",] # get row 1
```
```{r run_matrixNamedColumn}
m[, "c2"] # get column 2
...
Run:
```{python run_0IndexSetup}
x = [3.5, 0.4, 9.1, 7.7] #this is in python
```
```{python run_0Index0}
print(x[0])
```
```{python run_0IndexItem}
print(x[3])
```
```{python run_0IndexOutofBounds}
print(x[4]) # accessing indices larger than the list length raises error
```
...
Run:
```{r run_subVectorPosSetup}
x <- c(3.5, 0.4, 9.1, 7.7)
x[-1] # return all but the first element
```
```{r run_subVectorPosSubset}
x[-c(2,4)] # return the first and third element
```
...
x[c(FALSE,TRUE,TRUE,FALSE)]
```
```{r run_logVecGreaterThan}
x > 3
```
```{r run_logVecTrueOnly}
x[x > 3]
```
...
Run:
```{r run_matrixSubSetup}
x <- matrix(1:9, nrow=3, byrow=TRUE)
x
```
```{r run_matrixSubsetRowsColumns}
x[c(1,2), c(1,2)] # first two rows, first two columns
```
...
```{r run_matrixSubsetRowsOnly}
x[-c(1,3), ] # leaving a selector blank selects all, second row and all columns
```
```{r run_matrixSubsetColOnly}
x[, c(2,3)] # all rows included, last two columns
```
```{r run_matrixSubsetSecondRowSecondColumn}
x[-c(1,3), c(2)] # select the second row and second column
```
...
* If you find yourself copying and pasting code from one part of your script to
  another, you are repeating yourself!
